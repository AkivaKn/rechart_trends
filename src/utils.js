...
export async function convertCurrencyToEuros(productBenchmarksData) {
    ...
    const productBenchmarksInEuros = productBenchmarksWithYearKey.map((product) => {
    if (product.currency.id === 3) {
        ...
        const convertedPayment = product.payment * conversionRate[0].exchange_rate;
        const convertedBenchmark = product.benchmark * conversionRate[0].exchange_rate;
...
export function calculateTotalsByProvider(productBenchmarks) {
    const benchmarksByProvider = categoriseBenchmarks(productBenchmarks, 'provider_name');
    ...
    for (const provider in benchmarksByProvider) {
        ...
        totalsByProvider.push({
            ...
            totalPayment,totalBenchmark
        })
    }
    return totalsByProvider;
}
export function showProductPaymentTrends(productBenchmarks) {
    const benchmarksByProduct = categoriseBenchmarks(productBenchmarks, 'product_name');
    const paymentTrends = [];
    for (const product in benchmarksByProduct) {
        if (benchmarksByProduct[product].length > 1) {
            paymentTrends.push({product_name:product,product_trends:benchmarksByProduct[product]})
        }
    }
